...
## 12. 终端乱码与启动指南 (Windows)

针对 Windows 环境下终端（CMD/PowerShell）显示中文乱码及任务队列卡顿的问题，请遵循以下指南：

### 终端中文乱码修复
已在 `electron/rag/rag_task.ts` 中集成了自适应编码修复方案：
1. **自动检测**：系统会自动识别终端编码（CP936/GBK 或 UTF-8）。
2. **智能转码**：使用 `iconv-lite` 将日志输出转换为终端对应的编码格式。
3. **无需手动设置**：不再需要手动输入 `chcp 65001`，后端服务启动时会自动适配。

**验证方法**：
启动后端服务后，观察控制台输出。如果能正确显示中文文件名（如 `[RagTask] [测试文档.md] ...`），说明修复生效。

### 应用启动步骤 (标准流程)
请严格遵循以下步骤启动应用，以避免端口冲突和环境问题：

1. **环境准备**：
   - 确保已安装 Yarn (推荐 v1.22+ 或 v4.x)。
   - 确保端口 `5173` (前端) 和 `7071` (后端) 未被占用。

2. **启动后端 (Electron)**：
   - 打开终端 #1
   - 运行：`yarn dev-electron`
   - 等待输出 `Electron app started` 及后端服务启动日志。
...
4. **任务队列卡顿处理**：
   - 如果发现 RAG 任务队列卡住（如解析某文件后不再继续），请**重启后端服务**。
   - 后端启动时会自动执行 `resetStuckTasks()`，将状态为“处理中”的任务重置为“失败”，从而恢复队列流动。
   - 检查 `C:\AingDesk\logs` 下的错误日志，确认特定文件是否存在格式问题。
